function challs_flash_update() { //Flash 初始化函数
    ...
    a.title = "上传附件"; //设置组件头部名称

    a.FormName = "Filedata";
    //设置Form表单的文本域的Name属性

    a.url = "../FileUpload/update.aspx";
    //设置服务器接收代码文件

    a.parameter = "";
    //设置提交参数，以GET形式提交,例："key=value&key=value&..."
    a.typefile = ["DOC(*.doc;*.docx;*.xls;*.xlsx;*.pdf;*.txt)","*.doc;*.docx;*.xls;*.xlsx;*.pdf;*.txt"];
    ...
    //2个为一组，可以设置多组文件类型
    a.newTypeFile =["DOC(*.doc;*.docx;*.xls;*.xlsx;*.pdf;*.txt)","*.doc;*.docx;*.xls;*.xlsx;*.pdf;*.txt"];
